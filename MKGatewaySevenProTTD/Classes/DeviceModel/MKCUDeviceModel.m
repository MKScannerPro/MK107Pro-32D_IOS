...
NSString *const MKCUDeviceModelOfflineNotification = @"MKCUDeviceModelOfflineNotification";

@interface MKCUDeviceModel ()
...
@implementation MKCUDeviceModel

- (void)dealloc{
    NSLog(@"MKCUDeviceModel销毁");
}

#pragma mark - public method

- (void)startStateMonitoringTimer{
    if (self.onLineState == MKCUDeviceModelStateOffline) {
        return;
    }
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.onLineState = MKCUDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKCUDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"macAddress":self.macAddress}];
                if ([self.delegate respondsToSelector:@selector(cu_deviceOfflineWithMacAddress:)]) {
                    [self.delegate cu_deviceOfflineWithMacAddress:self.macAddress];
                }
            });
            return ;
        }
...
- (NSString *)currentSubscribedTopic {
    if (ValidStr([MKCUMQTTDataManager shared].serverParams.publishTopic)) {
        return [MKCUMQTTDataManager shared].serverParams.publishTopic;
    }
...
- (NSString *)currentPublishedTopic {
    if (ValidStr([MKCUMQTTDataManager shared].serverParams.subscribeTopic)) {
        return [MKCUMQTTDataManager shared].serverParams.subscribeTopic;
    }
