...
/**
 线程ID
 */
@property (nonatomic, assign)mk_cu_serverOperationID operationID;
...
@implementation MKCUMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKCUMQTTOperation销毁");
}

- (instancetype)initOperationWithID:(mk_cu_serverOperationID)operationID
...
#pragma mark - MKCUMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKCUMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_cu_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_cu_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
