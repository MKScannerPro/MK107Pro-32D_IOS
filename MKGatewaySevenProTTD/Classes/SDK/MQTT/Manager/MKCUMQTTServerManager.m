...
static MKCUMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKCUMQTTServerManager)

@end

@implementation NSObject (MKCUMQTTServerManager)

//+ (void)load{
//    [MKCUMQTTServerManager shared];
//}

@end

@interface MKCUMQTTServerManager ()

@property (nonatomic, assign)MKCUMQTTSessionManagerState state;

@property (nonatomic, strong)MKCUServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKCUServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKCUMQTTServerManager
...
+ (MKCUMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCUMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKCUServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cu_didReceiveMessage:onTopic:)]) {
                [protocol cu_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKCUServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cu_didChangeState:)]) {
                [protocol cu_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKCUMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKCUServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKCUServerManagerProtocol)]
...
- (BOOL)removeDataManager:(nonnull id <MKCUServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (!dataManager ||
            ![dataManager conformsToProtocol:@protocol(MKCUServerManagerProtocol)] ||
            ![self.managerList containsObject:dataManager]) {
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_cu_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cu_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cu_mqtt_recordDate"];
...
#pragma mark - getter
- (MKCUServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKCUServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKCUServerManagerProtocol>> *)managerList {
    if (!_managerList) {
