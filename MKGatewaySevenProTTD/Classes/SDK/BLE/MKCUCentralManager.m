...
static NSString *const mk_cu_logName = @"mk_cu_bleLog";

NSString *const mk_cu_peripheralConnectStateChangedNotification = @"mk_cu_peripheralConnectStateChangedNotification";
NSString *const mk_cu_centralManagerStateChangedNotification = @"mk_cu_centralManagerStateChangedNotification";

NSString *const mk_cu_deviceDisconnectTypeNotification = @"mk_cu_deviceDisconnectTypeNotification";

static MKCUCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKCUCentralManager ()
...
@property (nonatomic, assign)mk_cu_centralConnectStatus connectStatus;

@end

@implementation MKCUCentralManager

- (void)dealloc {
    [self logToLocal:@"MKCUCentralManager销毁"];
    NSLog(@"MKCUCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKCUCentralManager初始化"];
        [[MKBLEBaseCentralManager shared] loadDataManager:self];
    }
    return self;
}

+ (MKCUCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCUCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_cu_receiveDevice:)]) {
                [self.delegate mk_cu_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_cu_startScan)]) {
        [self.delegate mk_cu_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_cu_stopScan)]) {
        [self.delegate mk_cu_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_cu_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_cu_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_cu_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_cu_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_cu_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_cu_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_cu_deviceDisconnectTypeNotification
                                                            object:nil
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA03"]]) {
        if ([[string substringWithRange:NSMakeRange(0, 2)] isEqualToString:@"ee"]) {
            ...
            if ([cmd isEqualToString:@"51"]) {
                //扫描的wifi数据
                if ([self.wifiDelegate respondsToSelector:@selector(mk_cu_receiveWifi:)]) {
                    [self.wifiDelegate mk_cu_receiveWifi:[string substringFromIndex:6]];
                }
                return;
            }
        }
...
- (mk_cu_centralManagerStatus )centralStatus {
    return ([MKBLEBaseCentralManager shared].centralStatus == MKCentralManagerStateEnable)
    ? mk_cu_centralManagerStatusEnable
    : mk_cu_centralManagerStatusUnable;
}
...
- (void)addTaskWithTaskID:(mk_cu_taskOperationID)operationID
           characteristic:(CBCharacteristic *)characteristic
              commandData:(NSString *)commandData
             successBlock:(void (^)(id returnData))successBlock
             failureBlock:(void (^)(NSError *error))failureBlock {
    MKCUOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_cu_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKCUOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)addOperation:(MKCUOperation *)operation {
    if (!operation) {
        return;
    }
    [[MKBLEBaseCentralManager shared] addOperation:operation];
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKCUPeripheral *trackerPeripheral = [[MKCUPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKCUOperation *operation = [[MKCUOperation alloc] initOperationWithID:mk_cu_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.cu_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_cu_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_cu_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKCUOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_cu_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKCUOperation <MKBLEBaseOperationProtocol>*operation = [[MKCUOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKCUOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_cu_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (NSDictionary *)parseModelWithRssi:(NSNumber *)rssi advDic:(NSDictionary *)advDic peripheral:(CBPeripheral *)peripheral {
    ...
    if (![deviceType isEqualToString:@"60"] && ![deviceType isEqualToString:@"61"]) {
        //60为V1版本固件，61为V2版本固件
        return @{};
    }
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_cu_logName dataList:@[string]];
}
