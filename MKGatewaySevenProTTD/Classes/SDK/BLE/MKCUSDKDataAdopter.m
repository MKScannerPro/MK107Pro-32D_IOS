...
@implementation MKCUSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_cu_wifiSecurity)security {
    switch (security) {
        case mk_cu_wifiSecurity_personal:
            return @"00";
        case mk_cu_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_cu_eapType)eapType {
    switch (eapType) {
        case mk_cu_eapType_peap_mschapv2:
            return @"00";
        case mk_cu_eapType_ttls_mschapv2:
            return @"01";
        case mk_cu_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_cu_connectMode)mode {
    switch (mode) {
        case mk_cu_connectMode_TCP:
            return @"00";
        case mk_cu_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_cu_connectMode_CACertificate:
            return @"02";
        case mk_cu_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_cu_mqttServerQosMode)mode {
    switch (mode) {
        case mk_cu_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_cu_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_cu_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
