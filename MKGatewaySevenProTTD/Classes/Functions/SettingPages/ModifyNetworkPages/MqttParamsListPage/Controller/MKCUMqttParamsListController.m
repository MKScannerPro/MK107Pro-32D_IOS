//
//  MKCUMqttParamsListController.m
...
@interface MKCUMqttParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKCUMqttParamsModel *dataModel;

@end

@implementation MKCUMqttParamsListController

- (void)dealloc {
    NSLog(@"MKCUMqttParamsListController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKCUMqttWifiSettingsController *vc = [[MKCUMqttWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT Settings
        MKCUMqttServerController *vc = [[MKCUMqttServerController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)updateLocaDeviceData {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCUDeviceDatabaseManager updateClientID:self.dataModel.clientID subscribedTopic:self.dataModel.subscribeTopic publishedTopic:self.dataModel.publishTopic lwtStatus:self.dataModel.lwtStatus lwtTopic:self.dataModel.lwtTopic macAddress:[MKCUDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Setup succeed!"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cu_deviceModifyMQTTServerSuccessNotification"
                                                            object:nil
                                                          userInfo:@{
            @"macAddress":SafeStr([MKCUDeviceModeManager shared].macAddress),
...
- (MKCUMqttParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUMqttParamsModel alloc] init];
    }
    return _dataModel;
}
