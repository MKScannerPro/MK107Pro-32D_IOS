...
@implementation MKCUMqttServerSSLViewModel
@end

@interface MKCUMqttServerSSLView ()<MKCUMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCUMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCUMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCUMqttServerSSLTextField *clientCertView;

@end

@implementation MKCUMqttServerSSLView
...
#pragma mark - MKCUMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)cu_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate cu_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate cu_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate cu_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCUMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCUMqttServerSSLTextFieldModel *caModel = [[MKCUMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCUMqttServerSSLTextFieldModel *clientKeyModel = [[MKCUMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCUMqttServerSSLTextFieldModel *clientModel = [[MKCUMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUMqttServerSSLView", @"cu_switchSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUMqttServerSSLView", @"cu_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMqttServerSSLView", @"cu_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCUMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCUMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCUMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCUMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCUMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCUMqttServerSSLTextField alloc] init];
