//
//  MKCUMqttServerConfigFooterView.m
...
@implementation MKCUMqttServerConfigFooterViewModel
@end
...
@interface MKCUMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCUMqttServerSSLViewDelegate,
MKCUMqttServerSettingViewDelegate,
MKCUMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKCUMqttServerSSLView *sslView;

@property (nonatomic, strong)MKCUMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKCUMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCUMqttServerConfigFooterView
...
#pragma mark - MKCUMqttServerSSLViewDelegate
- (void)cu_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)cu_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)cu_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cu_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKCUMqttServerSettingViewDelegate
...
#pragma mark - MKCUMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKCUMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKCUMqttServerSSLViewModel *sslModel = [[MKCUMqttServerSSLViewModel alloc] init];
    ...
    MKCUMqttServerLwtViewModel *lwtModel = [[MKCUMqttServerLwtViewModel alloc] init];
...
- (MKCUMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCUMqttServerSSLView alloc] init];
...
- (MKCUMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKCUMqttServerLwtView alloc] init];
...
- (MKCUMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKCUMqttServerSettingView alloc] init];
