...
@interface MKCUMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCUMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCUMqttServerModel *dataModel;

@property (nonatomic, strong)MKCUMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCUMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCUMqttServerController

- (void)dealloc {
    NSLog(@"MKCUMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCUReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKCUMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cu_selectedServerParams:(NSString *)fileName {
    [MKCUExcelDataManager parseDeviceExcel:fileName
        ...
        MKCUMqttServerModel *model = [MKCUMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCUMqttServerModel *model = [[MKCUMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMqttServerController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCUMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCUMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCUMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCUMqttServerConfigFooterViewModel alloc] init];
    }
