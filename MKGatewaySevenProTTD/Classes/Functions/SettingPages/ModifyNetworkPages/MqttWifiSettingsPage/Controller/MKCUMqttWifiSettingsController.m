//
//  MKCUMqttWifiSettingsController.m
...
@interface MKCUMqttWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUMqttWifiSettingsModel *dataModel;

@end

@implementation MKCUMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCUMqttWifiSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMqttWifiSettingsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
