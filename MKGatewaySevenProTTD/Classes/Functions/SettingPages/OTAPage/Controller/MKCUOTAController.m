...
@interface MKCUOTAController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUOTAPageModel *dataModel;

@end

@implementation MKCUOTAController

- (void)dealloc {
    NSLog(@"MKCUOTAController销毁");
        ...
        if (self.dataModel.otaType == 0) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKCUReceiveDeviceOTAResultNotification
                                                       object:nil];
        }else if (self.dataModel.otaType == 1) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKCUReceiveDeviceNpcOTAResultNotification
                                                       object:nil];
        }
...
- (MKCUOTAPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUOTAPageModel alloc] init];
    }
    return _dataModel;
}
