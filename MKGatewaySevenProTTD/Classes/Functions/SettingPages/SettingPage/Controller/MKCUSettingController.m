...
@interface MKCUSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUSettingModel *dataModel;

@property (nonatomic, copy)NSString *localNameAsciiStr;

@end

@implementation MKCUSettingController

- (void)dealloc {
    NSLog(@"MKCUSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Indicator settings
        MKCUIndicatorSettingsController *vc = [[MKCUIndicatorSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Network status report interval
        MKCUNetworkStatusController *vc = [[MKCUNetworkStatusController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Reconnect timeout
        MKCUReconnectTimeController *vc = [[MKCUReconnectTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Communicate timeout
        MKCUCommunicateController *vc = [[MKCUCommunicateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //System time
        MKCUSystemTimeController *vc = [[MKCUSystemTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 5) {
        //Advertise iBeacon
        MKCUAdvBeaconController *vc = [[MKCUAdvBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 6) {
        //Reset device by button
        MKCUResetByButtonController *vc = [[MKCUResetByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 7) {
        //Data report timout
        MKCUDataReportController *vc = [[MKCUDataReportController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //OTA
        MKCUOTAController *vc = [[MKCUOTAController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //Modify network settings
        MKCUMqttParamsListController *vc = [[MKCUMqttParamsListController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //Device information
        MKCUDeviceInfoController *vc = [[MKCUDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)rootButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Reboot Device" message:msg notificationName:@"mk_cu_needDismissAlert"];
}
...
- (void)resetButtonPressed {
    ...
    [alertView addAction:cancelAction];
    [alertView addAction:confirmAction];
    [alertView showAlertWithTitle:@"Reset Device" message:msg notificationName:@"mk_cu_needDismissAlert"];
}
...
- (void)configLocalName{
    ...
    self.localNameAsciiStr = SafeStr([MKCUDeviceModeManager shared].deviceName);
    MKAlertViewTextField *textField = [[MKAlertViewTextField alloc] initWithTextValue:SafeStr([MKCUDeviceModeManager shared].deviceName)
    ...
    [alertView showAlertWithTitle:@"Edit Local Name" message:msg notificationName:@"mk_cu_needDismissAlert"];
}
...
- (void)saveDeviceLocalName {
    ...
    [MKCUDeviceDatabaseManager updateLocalName:self.localNameAsciiStr
                                    macAddress:[MKCUDeviceModeManager shared].macAddress
                                      sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cu_deviceNameChangedNotification"
                                                            object:nil
                                                          userInfo:@{
                                                              @"macAddress":[MKCUDeviceModeManager shared].macAddress,
...
- (void)resetDevice {
    ...
    [MKCUMQTTInterface cu_resetDeviceWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self removeDevice];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)removeDevice {
    ...
    [MKCUDeviceDatabaseManager deleteDeviceWithMacAddress:[MKCUDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cu_deleteDeviceNotification"
                                                            object:nil
                                                          userInfo:@{@"macAddress":[MKCUDeviceModeManager shared].macAddress}];
        [self popToViewControllerWithClassName:@"MKCUDeviceListController"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
    }];
...
- (void)rebootDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_rebootDeviceWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configOutputSwitch:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_configOutputSwitch:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success!"];
...
- (void)configOutputControlByButton:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_configOutputControlByButton:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success!"];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUSettingController", @"cu_editIcon.png") forState:UIControlStateNormal];
...
- (MKCUSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUSettingModel alloc] init];
    }
    return _dataModel;
}
