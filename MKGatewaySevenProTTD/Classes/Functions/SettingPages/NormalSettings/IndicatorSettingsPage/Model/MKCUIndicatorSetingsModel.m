...
@interface MKCUIndicatorSetingsModel ()
...
@implementation MKCUIndicatorSetingsModel
...
- (BOOL)readIndicatorLightStatus {
    ...
    [MKCUMQTTInterface cu_readIndicatorLightStatusWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIndicatorLightStatus {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configIndicatorLightStatus:self macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
