...
@interface MKCUAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUAdvBeaconV2Model *dataModel;

@end

@implementation MKCUAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKCUAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUAdvBeaconV2Controller", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}
