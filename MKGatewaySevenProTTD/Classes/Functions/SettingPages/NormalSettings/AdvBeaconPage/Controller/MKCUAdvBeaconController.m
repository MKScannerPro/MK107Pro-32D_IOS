...
@interface MKCUAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUAdvBeaconModel *dataModel;

@end

@implementation MKCUAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCUAdvBeaconController销毁");
}
...
- (MKCUAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
