...
@interface MKCUSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCUSystemTimeCellDelegate,
...
@implementation MKCUSystemTimeController

- (void)dealloc {
    NSLog(@"MKCUSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCUNTPServerController *vc = [[MKCUNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCUSystemTimeCell *cell = [MKCUSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCUSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCUMQTTInterface cu_readDeviceUTCTimeWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCUMQTTInterface cu_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCUSystemTimeCellModel *cellModel = [[MKCUSystemTimeCellModel alloc] init];
    cellModel.index = 0;
