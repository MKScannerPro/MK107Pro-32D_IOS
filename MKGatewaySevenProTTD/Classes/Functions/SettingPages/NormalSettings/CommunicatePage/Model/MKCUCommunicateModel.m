...
@interface MKCUCommunicateModel ()
...
@implementation MKCUCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKCUMQTTInterface cu_readCommunicateTimeoutWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
