...
@interface MKCUReconnectTimeModel ()
...
@implementation MKCUReconnectTimeModel
...
- (BOOL)readReconnectTimeout {
    ...
    [MKCUMQTTInterface cu_readNetworkReconnectTimeoutWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReconnectTimeout {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configReconnectTimeout:[self.timeout integerValue] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
