...
static NSString *const localPasswordKey = @"mk_cu_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKCUScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_cu_centralManagerScanDelegate>
...
@implementation MKCUScanPageController

- (void)dealloc {
    NSLog(@"MKCUScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCUCentralManager shared] stopScan];
    [MKCUCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKCUCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKCUCentralManager shared].centralStatus != mk_cu_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKCUScanPageCell *cell = [MKCUScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_cu_centralManagerScanDelegate

- (void)mk_cu_receiveDevice:(NSDictionary *)deviceModel {
    MKCUScanPageModel *dataModel = [MKCUScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKCUCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKCUScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKCUScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKCUCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKCUScanPageModel *)deviecModel {
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUScanPageController", @"cu_scanRefresh.png");
    }
    return _refreshIcon;
}
