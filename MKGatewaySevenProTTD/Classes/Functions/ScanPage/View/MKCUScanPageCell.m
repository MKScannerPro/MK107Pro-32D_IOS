...
@implementation MKCUScanPageCell

+ (MKCUScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKCUScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCUScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUScanPageModel.class]) {
        return;
    }
