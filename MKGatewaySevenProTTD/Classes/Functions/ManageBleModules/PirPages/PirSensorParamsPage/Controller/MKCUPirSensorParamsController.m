//
//  MKCUPirSensorParamsController.m
...
@interface MKCUPirSensorParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUPirSensorParamsModel *dataModel;

@end

@implementation MKCUPirSensorParamsController

- (void)dealloc {
    NSLog(@"MKCUPirSensorParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUPirSensorParamsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUPirSensorParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUPirSensorParamsModel alloc] init];
    }
    return _dataModel;
}
