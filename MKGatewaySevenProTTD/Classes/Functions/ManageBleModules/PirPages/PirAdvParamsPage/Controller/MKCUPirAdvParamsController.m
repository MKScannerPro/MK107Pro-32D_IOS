...
@interface MKCUPirAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUPirAdvParamsModel *dataModel;

@end

@implementation MKCUPirAdvParamsController

- (void)dealloc {
    NSLog(@"MKCUPirAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUPirAdvParamsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUPirAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUPirAdvParamsModel alloc] init];
    }
    return _dataModel;
}
