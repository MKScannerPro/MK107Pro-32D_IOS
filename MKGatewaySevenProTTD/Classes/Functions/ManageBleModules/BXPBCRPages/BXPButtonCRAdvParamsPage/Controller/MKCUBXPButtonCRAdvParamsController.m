//
//  MKCUBXPButtonCRAdvParamsController.m
...
@interface MKCUBXPButtonCRAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBXPButtonCRAdvParamsModel *dataModel;

@end

@implementation MKCUBXPButtonCRAdvParamsController

- (void)dealloc {
    NSLog(@"MKCUBXPButtonCRAdvParamsController销毁");
}
...
- (void)cu_advNormalCell_setPressed:(NSInteger)index
    ...
    [MKCUMQTTInterface cu_bxpBtnCRConfigAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if (!ValidDict(advParams)) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUBXPButtonCRAdvParamsControllerCell"];
    }
...
- (MKCUBXPButtonCRAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPButtonCRAdvParamsModel alloc] init];
    }
    return _dataModel;
}
