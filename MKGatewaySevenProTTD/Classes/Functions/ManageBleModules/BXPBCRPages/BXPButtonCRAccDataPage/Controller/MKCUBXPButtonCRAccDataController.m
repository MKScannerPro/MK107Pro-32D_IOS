//
//  MKCUBXPButtonCRAccDataController.m
...
@interface MKCUBXPButtonCRAccDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUBXPButtonCRAccDataController

- (void)dealloc {
    NSLog(@"MKCUBXPButtonCRAccDataController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpBtnCRNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCUReceiveBXPBtnCRAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
