//
//  MKCUBXPCHistoricalTHDataController.m
...
@interface MKCUBXPCHistoricalTHDataController ()<MFMailComposeViewControllerDelegate,
MKCUBXPCHistoricalTHDataHeaderViewDelegate>

@property (nonatomic, strong)MKCUBXPCHistoricalTHDataHeaderView *headerView;
...
@implementation MKCUBXPCHistoricalTHDataController

- (void)dealloc {
    NSLog(@"MKCUBXPCHistoricalTHDataController销毁");
...
#pragma mark - MKCUBXPCHistoricalTHDataHeaderViewDelegate
- (void)cu_bxpcHistoricalHTDataHeaderView_syncButtonPressed:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_bxpBXPCNotifyHistoricalHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCUReceiveBXPCHistoricalHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
- (void)cu_bxpcHistoricalHTDataHeaderView_deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_bxpBXPCDeleteHistoricalHTDataWithBleMac:self.bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
        self.textView.text = @"";
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKCUBXPCHistoricalTHDataHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKCUBXPCHistoricalTHDataHeaderView alloc] init];
