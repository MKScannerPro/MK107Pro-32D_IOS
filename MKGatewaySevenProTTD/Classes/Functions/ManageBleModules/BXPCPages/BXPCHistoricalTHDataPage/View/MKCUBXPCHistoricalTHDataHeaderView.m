//
//  MKCUBXPCHistoricalTHDataHeaderView.m
...
@interface MKCUBXPCHistoricalTHDataHeaderView ()
...
@implementation MKCUBXPCHistoricalTHDataHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_bxpcHistoricalHTDataHeaderView_syncButtonPressed:)]) {
        [self.delegate cu_bxpcHistoricalHTDataHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_bxpcHistoricalHTDataHeaderView_deleteButtonPressed)]) {
        [self.delegate cu_bxpcHistoricalHTDataHeaderView_deleteButtonPressed];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_bxpcHistoricalHTDataHeaderView_exportButtonPressed)]) {
        [self.delegate cu_bxpcHistoricalHTDataHeaderView_exportButtonPressed];
    }
}
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPCHistoricalTHDataHeaderView", @"cu_threeAxisAcceLoadingIcon.png");
    }
    return _synIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPCHistoricalTHDataHeaderView", @"cu_slotExportEnableIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPCHistoricalTHDataHeaderView", @"cu_slotExportDeleteIcon.png") forState:UIControlStateNormal];
