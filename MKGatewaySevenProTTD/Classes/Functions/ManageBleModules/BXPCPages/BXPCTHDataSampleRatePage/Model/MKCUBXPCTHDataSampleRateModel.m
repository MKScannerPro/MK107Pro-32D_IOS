//
//  MKCUBXPCTHDataSampleRateModel.m
...
@interface MKCUBXPCTHDataSampleRateModel ()
...
@implementation MKCUBXPCTHDataSampleRateModel
...
- (BOOL)readTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKCUMQTTInterface cu_readBXPCTHDataSampleRateWithBleMacAddress:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKCUMQTTInterface cu_configBXPCSampleRate:[self.self.sampleRate integerValue] bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
