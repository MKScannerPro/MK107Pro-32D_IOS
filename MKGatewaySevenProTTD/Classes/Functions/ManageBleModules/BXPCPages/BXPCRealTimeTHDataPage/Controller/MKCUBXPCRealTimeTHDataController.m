//
//  MKCUBXPCRealTimeTHDataController.m
...
@interface MKCUBXPCRealTimeTHDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUBXPCRealTimeTHDataController

- (void)dealloc {
    NSLog(@"MKCUBXPCRealTimeTHDataController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpBXPCNotifyRealTimeHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCUReceiveBXPCRealTimeHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
