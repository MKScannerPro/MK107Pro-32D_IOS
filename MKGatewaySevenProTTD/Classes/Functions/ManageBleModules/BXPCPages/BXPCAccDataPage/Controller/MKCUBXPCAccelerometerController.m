//
//  MKCUBXPCAccelerometerController.m
...
@interface MKCUBXPCAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUBXPCAccelerometerController

- (void)dealloc {
    NSLog(@"MKCUBXPCAccelerometerController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpBXPCNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCUReceiveBXPCAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
