//
//  MKCUBXPButtonRemoteReminderController.m
...
@interface MKCUBXPButtonRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBXPButtonRemoteReminderModel *dataModel;

@end

@implementation MKCUBXPButtonRemoteReminderController

- (void)dealloc {
    NSLog(@"MKCUBXPButtonRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKCUMQTTInterface cu_bxpBtnLedRemoteReminderWithBleMac:self.bleMac blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCUBXPButtonRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPButtonRemoteReminderModel alloc] init];
    }
    return _dataModel;
}
