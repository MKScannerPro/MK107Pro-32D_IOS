...
@implementation MKCUButtonFirmwareCellModel
@end

@interface MKCUButtonFirmwareCell ()
...
@implementation MKCUButtonFirmwareCell

+ (MKCUButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKCUButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate cu_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUButtonFirmwareCellModel.class]) {
        return;
    }
