//
//  MKCUManageBleDevicesV2Controller.m
...
@interface MKCUManageBleDevicesV2Controller ()<UITableViewDelegate,
UITableViewDataSource,
MKCUManageBleDevicesSearchButtonDelegate,
MKCUManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKCUManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKCUManageBleDevicesSearchButton *searchButton;
...
@implementation MKCUManageBleDevicesV2Controller

- (void)dealloc {
    NSLog(@"MKCUManageBleDevicesV2Controller销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCUManageBleDevicesCell *cell = [MKCUManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCUManageBleDevicesSearchButtonDelegate
- (void)cu_scanSearchButtonMethod {
    [MKCUManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
- (void)cu_scanSearchButtonClearMethod {
...
#pragma mark - MKCUManageBleDevicesCellDelegate
/// 用户点击了链接按钮
...
/*
 ...
 10：other
 */
- (void)cu_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode {
    
    NSString *recordSelectedType = [[NSUserDefaults standardUserDefaults] objectForKey:@"cu_bleDeviceConnectType"];
    if (!ValidStr(recordSelectedType)) {
        [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:@"cu_bleDeviceConnectType"];
        recordSelectedType = @"1";
    }
    
    [MKCUManageBleDevicesTypeSelectedView showWithType:[recordSelectedType integerValue] selecteBlock:^(MKCUManageBleDevicesTypeSelectedViewType selectedType) {
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%ld",(long)selectedType] forKey:@"cu_bleDeviceConnectType"];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKCUReceiveDeviceDatasNotification
                                               object:nil];
}
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKCUManageBleDevicesCellModel *cellModel = [[MKCUManageBleDevicesCellModel alloc] init];
...
- (void)connectDeviceWithMacAddress:(NSString *)macAddress
                       selectedType:(MKCUManageBleDevicesTypeSelectedViewType)selectedType {
    if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeOther) {
    ...
    MKAlertViewAction *confirmAction = [[MKAlertViewAction alloc] initWithTitle:@"OK" handler:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeBXPBD) {
                //BXP-B-D
                [self connectBXPButtonWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeBXPBCR) {
                //BXP-B-CR
                [self connectBXPButtonCRWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeBXPC) {
                //BXP-C
                [self connectBXPCWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeBXPD) {
                //BXP-D
                [self connectBXPDWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeBXPT) {
                //BXP-T
                [self connectBXPTWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeBXPS) {
                //BXP-S
                [self connectBXPSWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypePIR) {
                //MK Pir
                [self connectMKPirWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCUManageBleDevicesTypeSelectedViewTypeTOF) {
                //MK Tof
    ...
    [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_cu_needDismissAlert"];
}

- (void)connectBXPButtonWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)connectBXPButtonCRWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectBXPButtonCRWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPCWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectBXPCWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPDWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectBXPDWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPTWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectBXPTWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPSWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectBXPSWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectMKPirWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectMKPirWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectMKTofWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectMKTofWithPassword:password bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectNormalDeviceWithBleMac:(NSString *)macAddress {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_connectNormalBleDeviceWithBleMac:macAddress macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (MKCUManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKCUManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKCUManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKCUManageBleDevicesSearchButtonModel alloc] init];
