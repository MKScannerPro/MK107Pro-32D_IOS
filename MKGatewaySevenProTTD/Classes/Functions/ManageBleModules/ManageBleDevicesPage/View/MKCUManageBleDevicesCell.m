...
@implementation MKCUManageBleDevicesCellModel
@end

@interface MKCUManageBleDevicesCell ()
...
@implementation MKCUManageBleDevicesCell

+ (MKCUManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKCUManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate cu_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUManageBleDevicesCellModel.class]) {
        return;
    }
