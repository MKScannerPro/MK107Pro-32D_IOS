//
//  MKCUBXPTAccelerometerController.m
...
@interface MKCUBXPTAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUBXPTAccelerometerController

- (void)dealloc {
    NSLog(@"MKCUBXPTAccelerometerController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpBXPTNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCUReceiveBXPTAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
