//
//  MKCUBXPTRemoteReminderController.m
...
@interface MKCUBXPTRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBXPTRemoteReminderModel *dataModel;

@end

@implementation MKCUBXPTRemoteReminderController

- (void)dealloc {
    NSLog(@"MKCUBXPTRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKCUMQTTInterface cu_bxpBXPTLedRemoteReminderWithBleMac:self.bleMac color:self.dataModel.color blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCUBXPTRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPTRemoteReminderModel alloc] init];
    }
    return _dataModel;
}
