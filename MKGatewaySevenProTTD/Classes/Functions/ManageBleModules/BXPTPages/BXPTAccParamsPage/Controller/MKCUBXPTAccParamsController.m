...
@interface MKCUBXPTAccParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBXPTAccParamsModel *dataModel;

@end

@implementation MKCUBXPTAccParamsController

- (void)dealloc {
    NSLog(@"MKCUBXPTAccParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPTAccParamsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBXPTAccParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPTAccParamsModel alloc] init];
    }
    return _dataModel;
}
