...
@interface MKCUBXPDAccParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBXPDAccParamsModel *dataModel;

@end

@implementation MKCUBXPDAccParamsController

- (void)dealloc {
    NSLog(@"MKCUBXPDAccParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPDAccParamsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBXPDAccParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPDAccParamsModel alloc] init];
    }
    return _dataModel;
}
