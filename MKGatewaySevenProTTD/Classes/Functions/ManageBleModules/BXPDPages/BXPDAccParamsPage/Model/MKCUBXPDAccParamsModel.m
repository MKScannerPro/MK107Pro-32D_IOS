...
@interface MKCUBXPDAccParamsModel ()
...
@implementation MKCUBXPDAccParamsModel
...
- (BOOL)readAccAdvParams:(NSString *)bleMac {
    ...
    [MKCUMQTTInterface cu_readBXPDAccParamsWithBleMacAddress:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAccAdvParams:(NSString *)bleMac {
    ...
    [MKCUMQTTInterface cu_configBXPDAccParamsWithScale:self.scale sampleRate:self.sampleRate sensitivity:[self.sensitivity integerValue] bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
