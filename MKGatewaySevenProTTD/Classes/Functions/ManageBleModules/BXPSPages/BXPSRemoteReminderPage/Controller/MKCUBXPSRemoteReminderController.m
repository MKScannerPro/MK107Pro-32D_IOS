//
//  MKCUBXPSRemoteReminderController.m
...
@interface MKCUBXPSRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBXPSRemoteReminderModel *dataModel;

@end

@implementation MKCUBXPSRemoteReminderController

- (void)dealloc {
    NSLog(@"MKCUBXPSRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKCUMQTTInterface cu_bxpBXPSLedRemoteReminderWithBleMac:self.bleMac color:self.dataModel.color blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCUBXPSRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPSRemoteReminderModel alloc] init];
    }
    return _dataModel;
}
