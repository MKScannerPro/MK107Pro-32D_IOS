...
@implementation MKCUBXPSAdvTriggerCellModel

- (CGFloat)fetchCellHeight {
    if (self.slotType == MKCUBXPSAdvTriggerCellSlotTypeNoData) {
        return 44.f;
    }
    return 150.f;
}
...
@interface MKCUBXPSAdvTriggerCell ()
...
@implementation MKCUBXPSAdvTriggerCell

+ (MKCUBXPSAdvTriggerCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUBXPSAdvTriggerCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUBXPSAdvTriggerCellIdenty"];
    if (!cell) {
        cell = [[MKCUBXPSAdvTriggerCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUBXPSAdvTriggerCellIdenty"];
    }
    return cell;
}
...
- (void)layoutSubviews {
    ...
    if (self.dataModel.slotType == MKCUBXPSAdvTriggerCellSlotTypeNoData) {
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_BXPSAdvTriggerCell_setPressed:interval:txPower:)]) {
        [self.delegate cu_BXPSAdvTriggerCell_setPressed:self.dataModel.slotIndex
...
#pragma mark - setter
- (void)setDataModel:(MKCUBXPSAdvTriggerCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUBXPSAdvTriggerCellModel.class]) {
        return;
    }
    ...
    if (_dataModel.slotType == MKCUBXPSAdvTriggerCellSlotTypeNoData) {
...
- (NSString *)getSlotTypeString:(MKCUBXPSAdvTriggerCellSlotType)slotType {
    if (slotType == MKCUBXPSAdvTriggerCellSlotTypeUID) {
        return @"UID";
    }
    if (slotType == MKCUBXPSAdvTriggerCellSlotTypeURL) {
        return @"URL";
    }
    if (slotType == MKCUBXPSAdvTriggerCellSlotTypeTLM) {
        return @"TLM";
    }
    if (slotType == MKCUBXPSAdvTriggerCellSlotTypeBeacon) {
        return @"iBeacon";
    }
    if (slotType == MKCUBXPSAdvTriggerCellSlotTypeSensorInfo) {
        return @"Sensor info";
    }
    if (slotType == MKCUBXPSAdvTriggerCellSlotTypeTHInfo) {
        return @"T&H info";
    }
