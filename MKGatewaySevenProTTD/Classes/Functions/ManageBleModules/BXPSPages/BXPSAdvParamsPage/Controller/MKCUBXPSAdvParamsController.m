...
@interface MKCUBXPSAdvParamsController ()<UITableViewDelegate,
UITableViewDataSource,
MKCUBXPSAdvNormalCellDelegate,
MKCUBXPSAdvTriggerCellDelegate,
MKCUBXPSAdvTriggerTwoStateCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKCUBXPSAdvParamsModel *dataModel;

@end

@implementation MKCUBXPSAdvParamsController

- (void)dealloc {
    NSLog(@"MKCUBXPSAdvParamsController销毁");
}
...
#pragma mark - MKCUBXPSAdvNormalCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
...
/*
 ...
 6
 */
- (void)cu_BXPSAdvNormalCell_setPressed:(NSInteger)index
    ...
    [MKCUMQTTInterface cu_configBXPSAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
#pragma mark - MKCUBXPSAdvTriggerCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
///   - interval: 当前ADV interval
...
#pragma mark - MKCUBXPSAdvTriggerTwoStateCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKCUBXPSAdvNormalCellModel.class]) {
        //正常广播
        MKCUBXPSAdvNormalCell *cell = [MKCUBXPSAdvNormalCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    if ([obj isKindOfClass:MKCUBXPSAdvTriggerCellModel.class]) {
        //触发广播
        MKCUBXPSAdvTriggerCell *cell = [MKCUBXPSAdvTriggerCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    if ([obj isKindOfClass:MKCUBXPSAdvTriggerTwoStateCellModel.class]) {
        //触发前+触发后广播
        MKCUBXPSAdvTriggerTwoStateCell *cell = [MKCUBXPSAdvTriggerTwoStateCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUBXPSAdvParamsControllerIdenty"];
}
...
- (CGFloat)loadTableCellHeight:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKCUBXPSAdvNormalCellModel.class]) {
        //正常广播
        MKCUBXPSAdvNormalCellModel *tempModel = (MKCUBXPSAdvNormalCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    if ([obj isKindOfClass:MKCUBXPSAdvTriggerCellModel.class]) {
        //触发广播
        MKCUBXPSAdvTriggerCellModel *tempModel = (MKCUBXPSAdvTriggerCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    if ([obj isKindOfClass:MKCUBXPSAdvTriggerTwoStateCellModel.class]) {
        MKCUBXPSAdvTriggerTwoStateCellModel *tempModel = (MKCUBXPSAdvTriggerTwoStateCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    return 0.0f;
}
...
- (void)loadSectionDatas {
    for (NSInteger i = 0; i < self.dataModel.dataList.count; i ++) {
        ...
        if (!enable) {
            //通道关闭 No Data
            MKCUBXPSAdvNormalCellModel *cellModel = [[MKCUBXPSAdvNormalCellModel alloc] init];
            cellModel.slotIndex = channel;
            cellModel.slotType = MKCUBXPSAdvNormalCellSlotTypeNoData;
            ...
            if (channelType == 0) {
                ...
                MKCUBXPSAdvTriggerCellModel *cellModel = [[MKCUBXPSAdvTriggerCellModel alloc] init];
                cellModel.slotIndex = channel;
                ...
                NSDictionary *afterAdv = dic[@"trigger_after_adv"];
                MKCUBXPSAdvTriggerTwoStateCellModel *cellModel = [[MKCUBXPSAdvTriggerTwoStateCellModel alloc] init];
                cellModel.slotIndex = channel;
...
- (MKCUBXPSAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBXPSAdvParamsModel alloc] init];
    }
    return _dataModel;
}
