//
//  MKCUBXPSAccelerometerController.m
...
@interface MKCUBXPSAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUBXPSAccelerometerController

- (void)dealloc {
    NSLog(@"MKCUBXPSAccelerometerController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpBXPSNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCUReceiveBXPSAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
