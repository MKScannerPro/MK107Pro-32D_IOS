//
//  MKCUBXPSRealTimeTHDataController.m
...
@interface MKCUBXPSRealTimeTHDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUBXPSRealTimeTHDataController

- (void)dealloc {
    NSLog(@"MKCUBXPSRealTimeTHDataController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpBXPSNotifyRealTimeHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCUReceiveBXPSRealTimeHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
