//
//  MKCUBXPSHistoricalTHDataController.m
...
@interface MKCUBXPSHistoricalTHDataController ()<MFMailComposeViewControllerDelegate,
MKCUBXPSHistoricalTHDataHeaderViewDelegate>

@property (nonatomic, strong)MKCUBXPSHistoricalTHDataHeaderView *headerView;
...
@implementation MKCUBXPSHistoricalTHDataController

- (void)dealloc {
    NSLog(@"MKCUBXPSHistoricalTHDataController销毁");
...
#pragma mark - MKCUBXPSHistoricalTHDataHeaderViewDelegate
- (void)cu_BXPSHistoricalHTDataHeaderView_syncButtonPressed:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_bxpBXPSNotifyHistoricalHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCUReceiveBXPSHistoricalHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
- (void)cu_BXPSHistoricalHTDataHeaderView_deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_bxpBXPSDeleteHistoricalHTDataWithBleMac:self.bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
        self.textView.text = @"";
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKCUBXPSHistoricalTHDataHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKCUBXPSHistoricalTHDataHeaderView alloc] init];
