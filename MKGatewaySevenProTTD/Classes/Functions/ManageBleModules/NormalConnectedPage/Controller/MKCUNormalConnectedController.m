//
//  MKCUNormalConnectedController.m
...
@interface MKCUNormalConnectedController ()<UITableViewDelegate,
UITableViewDataSource,
MKCUNormalConnectedCellDelegate>
...
@property (nonatomic, strong)MKCUConnectedDeviceWriteAlertView *writeAlert;

@end

@implementation MKCUNormalConnectedController

- (void)dealloc {
    NSLog(@"MKCUNormalConnectedController销毁");
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKCUNormalConnectedCellModel *cellModel = charList[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCUNormalConnectedCell *cell = [MKCUNormalConnectedCell initCellWithTableView:tableView];
    NSDictionary *dic = self.sectionList[indexPath.section];
    NSArray *charList = dic[@"charList"];
...
#pragma mark - MKCUNormalConnectedCellDelegate
- (void)cu_normalConnectedCell_writeButtonPressed:(NSInteger)section
                                              row:(NSInteger)row
                                       serverUUID:(NSString *)serverUUID
                                   characteristic:(NSString *)characteristic {
    @weakify(self);
    [self.writeAlert showAlertWithValue:@"" dismissNote:@"mk_cu_needDismissAlert" cancelAction:^{
...
- (void)cu_normalConnectedCell_readButtonPressed:(NSInteger)section
                                             row:(NSInteger)row
                                      serverUUID:(NSString *)serverUUID
                                  characteristic:(NSString *)characteristic {
...
- (void)cu_normalConnectedCell_notifyButtonPressed:(BOOL)notify
...
- (void)receiveDeviceDatas:(NSNotification *)note {
    ...
    MKCUNormalConnectedCellModel *cellModel = charList[row];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDisconnect:)
                                                 name:MKCUReceiveGatewayDisconnectDeviceNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKCUReceiveGatewayConnectedDeviceDatasNotification
                                               object:nil];
}
...
- (void)parseTableDatas {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        for (NSInteger j = 0; j < charCount; j ++) {
            ...
            MKCUNormalConnectedCellModel *cellModel = [[MKCUNormalConnectedCellModel alloc] init];
            cellModel.section = i;
            cellModel.row = j;
...
- (void)writeData:(NSString *)data serviceUUID:(NSString *)serverUUID characteristic:(NSString *)characteristic {
    ...
    [MKCUMQTTInterface cu_writeValueToDeviceWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] value:data serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)readDataWithServiceUUID:(NSString *)serverUUID
    ...
    [MKCUMQTTInterface cu_readCharacteristicValueWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
characteristic:(NSString *)characteristic
    ...
    [MKCUMQTTInterface cu_notifyCharacteristic:notify bleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (MKCUConnectedDeviceWriteAlertView *)writeAlert {
    if (!_writeAlert) {
        _writeAlert = [[MKCUConnectedDeviceWriteAlertView alloc] init];
    }
    return _writeAlert;
}
