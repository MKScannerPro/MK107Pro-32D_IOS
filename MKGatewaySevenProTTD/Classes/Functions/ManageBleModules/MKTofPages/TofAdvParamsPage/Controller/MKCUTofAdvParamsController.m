...
@interface MKCUTofAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUTofAdvParamsModel *dataModel;

@end

@implementation MKCUTofAdvParamsController

- (void)dealloc {
    NSLog(@"MKCUTofAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUTofAdvParamsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUTofAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUTofAdvParamsModel alloc] init];
    }
    return _dataModel;
}
