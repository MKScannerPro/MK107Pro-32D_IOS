...
@interface MKCUTofSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCUTofSensorDataController

- (void)dealloc {
    NSLog(@"MKCUTofSensorDataController销毁");
...
- (void)cu_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_bxpMKTofNotifySensorDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveSensorDatas:)
                                                         name:MKCUReceiveMKTofDistanceDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
