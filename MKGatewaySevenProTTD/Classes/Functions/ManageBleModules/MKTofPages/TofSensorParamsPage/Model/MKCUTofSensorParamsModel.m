...
@interface MKCUTofSensorParamsModel ()
...
@implementation MKCUTofSensorParamsModel
...
- (BOOL)readSensorParams:(NSString *)bleMac {
    ...
    [MKCUMQTTInterface cu_readMKTofSensorParamsWithBleMacAddress:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configSensorParams:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configMKTofSensorParamsWithInterval:[self.interval integerValue] sampleCount:[self.count integerValue] sampleTime:[self.time integerValue] bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readDistanceMode:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_readMKTofRangingModeWithBleMacAddress:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDistanceMode:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configMKTofRangingMode:self.distanceMode bleMac:bleMac macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
