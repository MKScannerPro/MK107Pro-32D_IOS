...
@interface MKCUSyncDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKCUSyncDeviceCellDelegate>
...
@implementation MKCUSyncDeviceController

- (void)dealloc {
    NSLog(@"MKCUSyncDeviceController销毁");
}
...
- (void)rightButtonMethod {
    ...
    [alert showWithAccount:[MKCUUserLoginManager shared].username completeBlock:^{
        [[MKCUUserLoginManager shared] syncLoginDataWithHome:[MKCUUserLoginManager shared].isHome username:[MKCUUserLoginManager shared].username password:@""];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCUSyncDeviceCell *cell = [MKCUSyncDeviceCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCUSyncDeviceCellDelegate
- (void)cu_syncDeviceCell_selected:(BOOL)selected index:(NSInteger)index {
    MKCUSyncDeviceCellModel *cellModel = self.dataList[index];
...
- (void)syncButtonPressed {
    ...
    for (MKCUSyncDeviceCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            MKCUCreatScannerProDeviceModel *uploadModel = [[MKCUCreatScannerProDeviceModel alloc] init];
    ...
    [[MKCUNetworkService share] addScannerProDevicesToCloud:uploadList isHome:[MKCUUserLoginManager shared].isHome token:self.token sucBlock:^(id returnData) {
...
- (void)loadDataSections {
    for (NSInteger i = 0; i < self.deviceList.count; i ++) {
        MKCUDeviceModel *device = self.deviceList[i];
        MKCUSyncDeviceCellModel *cellModel = [[MKCUSyncDeviceCellModel alloc] init];
        cellModel.index = i;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUSyncDeviceController", @"cu_authorIcon.png") forState:UIControlStateNormal];
