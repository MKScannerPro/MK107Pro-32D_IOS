...
@implementation MKCUSyncDeviceCellModel
@end

@interface MKCUSyncDeviceCell ()
...
@implementation MKCUSyncDeviceCell

+ (MKCUSyncDeviceCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUSyncDeviceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUSyncDeviceCellIdenty"];
    if (!cell) {
        cell = [[MKCUSyncDeviceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUSyncDeviceCellIdenty"];
    }
    return cell;
}
...
- (void)backBtnPressed {
    ...
    [self updateIcon];
    if ([self.delegate respondsToSelector:@selector(cu_syncDeviceCell_selected:index:)]) {
        [self.delegate cu_syncDeviceCell_selected:self.backBtn.selected index:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUSyncDeviceCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUSyncDeviceCellModel.class]) {
        return;
    }
...
- (void)updateIcon {
    UIImage *btnIcon = (self.backBtn.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUSyncDeviceCell", @"cu_listButtonSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUSyncDeviceCell", @"cu_listButtonUnselectedIcon.png"));
