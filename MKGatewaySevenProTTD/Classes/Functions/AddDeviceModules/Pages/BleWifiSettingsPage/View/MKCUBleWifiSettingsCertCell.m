...
@implementation MKCUBleWifiSettingsCertCellModel
@end

@interface MKCUBleWifiSettingsCertCell ()
...
@implementation MKCUBleWifiSettingsCertCell

+ (MKCUBleWifiSettingsCertCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUBleWifiSettingsCertCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUBleWifiSettingsCertCellIdenty"];
    if (!cell) {
        cell = [[MKCUBleWifiSettingsCertCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUBleWifiSettingsCertCellIdenty"];
    }
    return cell;
}
...
- (void)certificateButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_bleWifiSettingsCertPressed:)]) {
        [self.delegate cu_bleWifiSettingsCertPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUBleWifiSettingsCertCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUBleWifiSettingsCertCellModel.class]) {
        return;
    }
...
- (UIButton *)certificateButton {
    if (!_certificateButton) {
        ...
        [_certificateButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBleWifiSettingsCertCell", @"cu_config_certAddIcon.png") forState:UIControlStateNormal];
