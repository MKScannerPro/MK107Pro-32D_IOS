//
//  MKCUBleWifiSettingsController.m
...
#include "MKCUBleWifiSettingsCertCell.h"
...
@interface MKCUBleWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKCUNetworkSsidSettingsCellDelegate,
MKCUBleWifiSettingsCertCellDelegate,
MKCAFileSelectControllerDelegate,
MKCUNearbyWifiControllerDelegate>
...
@property (nonatomic, strong)MKCUBleWifiSettingsModel *dataModel;

@end

@implementation MKCUBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCUBleWifiSettingsController销毁");
}
...
#pragma mark - MKCUNetworkSsidSettingsCellDelegate
- (void)cu_networkSsidSettingsCell_ssidChanged:(NSString *)ssid {
    //SSID
    self.dataModel.ssid = ssid;
    MKCUNetworkSsidSettingsCellModel *cellModel = self.section2List[0];
    cellModel.ssid = ssid;
}

- (void)cu_networkSsidSettingsCell_buttonPressed {
    MKCUNearbyWifiController *vc = [[MKCUNearbyWifiController alloc] init];
    vc.delegate = self;
    [self.navigationController pushViewController:vc animated:YES];
}

#pragma mark - MKCUNearbyWifiControllerDelegate
...
#pragma mark - MKCUBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKCUBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKCUBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKCUBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCUDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCUDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        //SSID
        if (self.isV2) {
            MKCUNetworkSsidSettingsCell *cell = [MKCUNetworkSsidSettingsCell initCellWithTableView:self.tableView];
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKCUBleWifiSettingsCertCell *cell = [MKCUBleWifiSettingsCertCell initCellWithTableView:self.tableView];
...
- (void)loadSection2Datas {
    if (self.isV2) {
        //V2
        MKCUNetworkSsidSettingsCellModel *cellModel = [[MKCUNetworkSsidSettingsCellModel alloc] init];
...
- (void)loadSection8Datas {
    MKCUBleWifiSettingsCertCellModel *cellModel = [[MKCUBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBleWifiSettingsController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
