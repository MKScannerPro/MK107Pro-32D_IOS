...
@interface MKCUNearbyWifiController ()<UITableViewDelegate,
UITableViewDataSource,
mk_cu_centralManagerScanWifiDelegate>
...
@implementation MKCUNearbyWifiController

- (void)dealloc {
    NSLog(@"MKCUNearbyWifiController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCUNearbyWifiCellModel *cellModel = self.dataList[indexPath.row];
    if ([self.delegate respondsToSelector:@selector(cu_nearbyWifiController_selectedWifi:)]) {
        [self.delegate cu_nearbyWifiController_selectedWifi:cellModel.ssid];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCUNearbyWifiCell *cell = [MKCUNearbyWifiCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_cu_centralManagerScanWifiDelegate
...
- (void)startScanWifi {
    ...
    @weakify(self);
    [MKCUInterface cu_startWifiScanWithSucBlock:^{
        [MKCUCentralManager shared].wifiDelegate = self;
...
- (void)parseContentList {
    ...
    while (index < length) {
        ...
        // 创建模型对象
        MKCUNearbyWifiCellModel *model = [[MKCUNearbyWifiCellModel alloc] init];
        model.bssid = bssid;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUNearbyWifiController", @"cu_refreshWifiListIcon.png") forState:UIControlStateNormal];
