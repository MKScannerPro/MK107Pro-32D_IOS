...
@implementation MKCUNearbyWifiCellModel
@end

@interface MKCUNearbyWifiCell ()
...
@implementation MKCUNearbyWifiCell


+ (MKCUNearbyWifiCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUNearbyWifiCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUNearbyWifiCellIdenty"];
    if (!cell) {
        cell = [[MKCUNearbyWifiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUNearbyWifiCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCUNearbyWifiCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUNearbyWifiCellModel.class]) {
        return;
    }
    //顶部
