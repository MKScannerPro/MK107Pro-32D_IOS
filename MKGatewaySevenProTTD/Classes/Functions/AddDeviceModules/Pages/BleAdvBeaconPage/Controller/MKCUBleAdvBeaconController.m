...
@interface MKCUBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBleAdvBeaconModel *dataModel;

@end

@implementation MKCUBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCUBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBleAdvBeaconController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
