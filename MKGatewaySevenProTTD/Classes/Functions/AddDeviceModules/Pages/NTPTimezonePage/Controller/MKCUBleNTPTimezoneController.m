//
//  MKCUBleNTPTimezoneController.m
...
@interface MKCUBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKCUBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKCUBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBleNTPTimezoneController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
