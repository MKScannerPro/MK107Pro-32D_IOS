//
//  MKCUServerForDeviceController.m
...
@interface MKCUServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCUServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKCUImportServerControllerDelegate>
...
@property (nonatomic, strong)MKCUServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKCUServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCUServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCUServerForDeviceController

- (void)dealloc {
    NSLog(@"MKCUServerForDeviceController销毁");
}
...
#pragma mark - MKCUServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)cu_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cu_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)cu_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)cu_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)cu_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)cu_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKCUImportServerController *vc = [[MKCUImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKCUImportServerControllerDelegate
- (void)cu_selectedServerParams:(NSString *)fileName {
    [MKCUExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCUServerForDeviceModel *model = [MKCUServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCUDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCUDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKCUDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKCUDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKCUDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKCUDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKCUDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKCUDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKCUExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKCUServerForDeviceModel *emptyModel = [[MKCUServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUServerForDeviceController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCUServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCUServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCUServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCUServerConfigDeviceFooterViewModel alloc] init];
    }
