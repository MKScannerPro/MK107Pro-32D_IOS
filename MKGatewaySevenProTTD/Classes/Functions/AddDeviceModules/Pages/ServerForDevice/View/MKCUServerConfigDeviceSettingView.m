//
//  MKCUServerConfigDeviceSettingView.m
...
@interface MKCUServerConfigDeviceSettingView ()
...
@implementation MKCUServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cu_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cu_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cu_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
