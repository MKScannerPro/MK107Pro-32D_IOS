...
@implementation MKCUMQTTSSLForDeviceViewModel
@end

@interface MKCUMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCUMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate cu_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate cu_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate cu_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate cu_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate cu_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCUMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTSSLForDeviceView", @"cu_switchSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTSSLForDeviceView", @"cu_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTSSLForDeviceView", @"cu_switchUnselectedIcon.png") forState:UIControlStateNormal];
