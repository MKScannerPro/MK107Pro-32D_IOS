...
@implementation MKCUMQTTLWTForDeviceViewModel
@end

@interface MKCUMQTTLWTForDeviceView ()
...
@implementation MKCUMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTLWTForDeviceView", @"cu_switchSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTLWTForDeviceView", @"cu_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(cu_lwt_statusChanged:)]) {
        [self.delegate cu_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTLWTForDeviceView", @"cu_switchSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTLWTForDeviceView", @"cu_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(cu_lwt_retainChanged:)]) {
        [self.delegate cu_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(cu_lwt_qosChanged:)]) {
        [self.delegate cu_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTLWTForDeviceView", @"cu_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTLWTForDeviceView", @"cu_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cu_lwt_topicChanged:)]) {
                [self.delegate cu_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cu_lwt_payloadChanged:)]) {
                [self.delegate cu_lwt_payloadChanged:text];
            }
        };
