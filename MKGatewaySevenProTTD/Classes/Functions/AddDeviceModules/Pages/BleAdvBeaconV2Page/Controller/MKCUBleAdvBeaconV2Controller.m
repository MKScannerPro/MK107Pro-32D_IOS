//
//  MKCUBleAdvBeaconV2Controller.m
...
@interface MKCUBleAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBleAdvBeaconV2Model *dataModel;

@end

@implementation MKCUBleAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKCUBleAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBleAdvBeaconV2Controller", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBleAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBleAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}
