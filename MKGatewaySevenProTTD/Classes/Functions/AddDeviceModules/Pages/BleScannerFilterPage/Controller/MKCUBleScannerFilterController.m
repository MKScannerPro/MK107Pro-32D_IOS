//
//  MKCUBleScannerFilterController.m
...
@interface MKCUBleScannerFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUBleScannerFilterModel *dataModel;

@end

@implementation MKCUBleScannerFilterController

- (void)dealloc {
    NSLog(@"MKCUBleScannerFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBleScannerFilterController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUBleScannerFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUBleScannerFilterModel alloc] init];
    }
    return _dataModel;
}
