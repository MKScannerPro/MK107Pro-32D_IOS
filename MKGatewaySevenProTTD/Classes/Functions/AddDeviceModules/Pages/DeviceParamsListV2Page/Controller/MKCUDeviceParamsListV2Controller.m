//
//  MKCUDeviceParamsListV2Controller.m
...
@interface MKCUDeviceParamsListV2Controller ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKCUDeviceParamsListV2Controller

- (void)dealloc {
    NSLog(@"MKCUDeviceParamsListV2Controller销毁");
...
- (void)viewDidLoad {
    ...
    [MKCUDeviceMQTTParamsModel shared].deviceModel.deviceType = @"71";
...
- (void)leftButtonMethod {
    ...
    [MKCUDeviceMQTTParamsModel sharedDealloc];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Network Settings
        MKCUBleWifiSettingsController *vc = [[MKCUBleWifiSettingsController alloc] init];
        vc.isV2 = YES;
    ...
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT settings
        MKCUServerForDeviceController *vc = [[MKCUServerForDeviceController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 1 && indexPath.row == 0) {
        //NTP & Timezone
        MKCUBleNTPTimezoneController *vc = [[MKCUBleNTPTimezoneController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //Scanner Filter
        MKCUBleScannerFilterController *vc = [[MKCUBleScannerFilterController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Advertise iBeacon
        MKCUBleAdvBeaconV2Controller *vc = [[MKCUBleAdvBeaconV2Controller alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 3) {
        //Device Information
        MKCUBleDeviceInfoV2Controller *vc = [[MKCUBleDeviceInfoV2Controller alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)receiveDeviceOnline:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"macAddress"]) || ![[MKCUDeviceMQTTParamsModel shared].deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
        return;
    }
...
- (void)connectButtonPressed {
    if (!self.originMode) {
        ...
        [MKCUInterface cu_enterSTAModeWithSucBlock:^{
    ...
    if (![MKCUDeviceMQTTParamsModel shared].wifiConfig || ![MKCUDeviceMQTTParamsModel shared].mqttConfig) {
...
- (void)startMqttProcess {
    ...
    if (ValidStr([MKCUMQTTDataManager shared].serverParams.subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKCUMQTTDataManager shared].serverParams.subscribeTopic;
    }else {
        topic = [MKCUDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    }
    [self.progressView show];
    [[MKCUMQTTDataManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    MKCUDeviceModel *deviceModel = [[MKCUDeviceModel alloc] init];
    deviceModel.deviceType = [MKCUDeviceMQTTParamsModel shared].deviceModel.deviceType;
    deviceModel.clientID = [MKCUDeviceMQTTParamsModel shared].deviceModel.clientID;
    deviceModel.subscribedTopic = [MKCUDeviceMQTTParamsModel shared].deviceModel.subscribedTopic;
    deviceModel.publishedTopic = [MKCUDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    deviceModel.macAddress = [MKCUDeviceMQTTParamsModel shared].deviceModel.macAddress;
    deviceModel.deviceName = [MKCUDeviceMQTTParamsModel shared].deviceModel.deviceName;
    deviceModel.lwtStatus = [MKCUDeviceMQTTParamsModel shared].deviceModel.lwtStatus;
    deviceModel.lwtTopic = [MKCUDeviceMQTTParamsModel shared].deviceModel.lwtTopic;
        
    MKCUConnectSuccessController *vc = [[MKCUConnectSuccessController alloc] init];
