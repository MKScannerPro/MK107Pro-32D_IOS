...
@interface MKCUDeviceListController ()<UITableViewDelegate,
UITableViewDataSource,
MKCUDeviceListCellDelegate,
MKCUDeviceModelDelegate>

/// 没有添加设备的时候显示
@property (nonatomic, strong)MKCUAddDeviceView *addView;
...
@property (nonatomic, strong)MKCUEasyShowView *loadingView;
...
@implementation MKCUDeviceListController

- (void)dealloc {
    NSLog(@"MKCUDeviceListController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCUMQTTDataManager shared] clearAllSubscriptions];
    [[MKCUMQTTDataManager shared] disconnect];
    [MKCUMQTTDataManager singleDealloc];
    [MKCUMQTTServerManager singleDealloc];
}
...
- (void)viewDidLoad {
    ...
    if (self.connectServer) {
        ...
        [[MKCUMQTTServerManager shared] startWork];
    }
...
- (void)rightButtonMethod {
    MKCUServerForAppController *vc = [[MKCUServerForAppController alloc] init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKCUDeviceListModel *deviceModel = self.dataList[indexPath.row];
    if (deviceModel.onLineState != MKCUDeviceModelStateOnline) {
        [self.view showCentralToast:@"Device is off-line!"];
        return;
    }
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_readDeviceInfoWithMacAddress:deviceModel.macAddress topic:[deviceModel currentSubscribedTopic] sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [[MKCUDeviceModeManager shared] addDeviceModel:deviceModel];
        ...
        [MKCUDeviceModeManager shared].isV2 = ([firmware integerValue] >= 200);
        MKCUDeviceDataController *vc = [[MKCUDeviceDataController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCUDeviceListCell *cell = [MKCUDeviceListCell initCellWithTableView:tableView];
...
#pragma mark - MKCUDeviceListCellDelegate
/**
 删除
...
- (void)cu_cellDeleteButtonPressed:(NSInteger)index {
    ...
    [alertView showAlertWithTitle:@"Remove Device" message:msg notificationName:@"mk_cu_needDismissAlert"];
}

#pragma mark - MKCUDeviceModelDelegate
/// 当前设备离线
/// @param deviceID 当前设备的deviceID
- (void)cu_deviceOfflineWithMacAddress:(NSString *)macAddress {
    [self deviceModelOnlineStateChanged:MKCUDeviceModelStateOffline macAddress:macAddress];
}
...
- (void)receiveNewDevice:(NSNotification *)note {
    ...
    MKCUDeviceModel *receiveModel = user[@"deviceModel"];
    MKCUDeviceListModel *deviceModel = [[MKCUDeviceListModel alloc] init];
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCUDeviceListModel *model = self.dataList[i];
    ...
    [[MKCUMQTTDataManager shared] subscriptions:topicList];
}
...
- (void)receiveDeviceOnlineState:(NSNotification *)note {
    ...
    [self deviceModelOnlineStateChanged:MKCUDeviceModelStateOnline macAddress:user[@"macAddress"]];
}
...
- (void)receiveDeviceNetworkState:(NSNotification *)note {
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCUDeviceListModel *deviceModel = self.dataList[i];
        if ([deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
            deviceModel.onLineState = MKCUDeviceModelStateOnline;
            [deviceModel startStateMonitoringTimer];
...
- (void)receiveDeleteDevice:(NSNotification *)note {
    ...
    MKCUDeviceListModel *deviceModel = nil;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCUDeviceListModel *model = self.dataList[i];
    ...
    [[MKCUMQTTDataManager shared] unsubscriptions:unSubTopicList];
...
- (void)receiveDeviceModifyMQTTServer:(NSNotification *)note {
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCUDeviceModel *model = self.dataList[i];
        if ([model.macAddress isEqualToString:user[@"macAddress"]]) {
            if (!ValidStr([MKCUMQTTDataManager shared].serverParams.subscribeTopic)) {
            ...
            model.onLineState = MKCUDeviceModelStateOffline;
...
- (void)resubTopics:(NSArray *)subTopicList {
    [self loadMainViews];
    [[MKCUMQTTDataManager shared] subscriptions:subTopicList];
}

/// 当前MQTT服务器连接状态发生改变
- (void)serverManagerStateChanged {
    if ([MKCUMQTTDataManager shared].state == MKCUMQTTSessionManagerStateConnecting) {
        [self.loadingView showText:@"Connecting..." superView:self.titleLabel animated:YES];
        return;
    }
    if ([MKCUMQTTDataManager shared].state == MKCUMQTTSessionManagerStateConnected) {
        [self.loadingView hidden];
        self.defaultTitle = @"MKScannerPro";
        return;
    }
    if ([MKCUMQTTDataManager shared].state == MKCUMQTTSessionManagerStateError) {
        [self.loadingView hidden];
...
- (void)receiveDeviceLwtMessage:(NSNotification *)note {
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        ...
        if ([deviceModel.macAddress isEqualToString:macAddress]) {
            deviceModel.onLineState = MKCUDeviceModelStateOffline;
            break;
        }
    }
    [self needRefreshList];
...
- (void)reloadDeviceTopics {
    ...
    NSMutableArray *topicList = [NSMutableArray array];
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCUDeviceModel *deviceModel = self.dataList[i];
        [topicList addObject:[deviceModel currentPublishedTopic]];
    }
    [[MKCUMQTTDataManager shared] subscriptions:topicList];
}
...
- (void)addButtonPressed {
    if (!ValidStr([MKCUMQTTDataManager shared].serverParams.host)) {
        //如果MQTT服务器参数不存在，则去引导用户添加服务器参数，让app连接MQTT服务器
        [self rightButtonMethod];
        return;
    }
    //MQTT服务器参数存在，则添加设备
    MKCUScanPageController *vc = [[MKCUScanPageController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)syncButtonPressed {
    ...
    if (ValidStr([MKCUUserLoginManager shared].password)) {
        //已经登陆过
        [self login:[MKCUUserLoginManager shared].isHome username:[MKCUUserLoginManager shared].username password:[MKCUUserLoginManager shared].password];
        return;
    }
    MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
    viewModel.account = [MKCUUserLoginManager shared].username;
    viewModel.isHome = [MKCUUserLoginManager shared].isHome;
    viewModel.password = [MKCUUserLoginManager shared].password;
...
- (void)readDataFromDatabase {
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKCUDeviceDatabaseManager readLocalDeviceWithSucBlock:^(NSArray<MKCUDeviceModel *> * _Nonnull deviceList) {
        [[MKHudManager share] hide];
...
- (void)loadTopics:(NSArray <MKCUDeviceModel *>*)deviceList {
    NSMutableArray *topicList = [NSMutableArray array];
    for (NSInteger i = 0; i < deviceList.count; i ++) {
        MKCUDeviceModel *tempModel = deviceList[i];
        MKCUDeviceListModel *deviceModel = [[MKCUDeviceListModel alloc] init];
...
- (void)deviceModelOnlineStateChanged:(MKCUDeviceModelState)state macAddress:(NSString *)macAddress {
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCUDeviceListModel *deviceModel = self.dataList[i];
        if ([deviceModel.macAddress isEqualToString:macAddress]) {
            deviceModel.onLineState = state;
            if (state == MKCUDeviceModelStateOnline) {
                //在线状态开启监听
                [deviceModel startStateMonitoringTimer];
            }
            break;
...
- (void)removeDeviceFromLocal:(NSInteger)index {
    MKCUDeviceListModel *deviceModel = self.dataList[index];
    [[MKHudManager share] showHUDWithTitle:@"Delete..." inView:self.view isPenetration:NO];
    [MKCUDeviceDatabaseManager deleteDeviceWithMacAddress:deviceModel.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [self.dataList removeObject:deviceModel];
        [[MKCUMQTTDataManager shared] unsubscriptions:@[[deviceModel currentPublishedTopic]]];
        [self loadMainViews];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveNewDevice:)
                                                 name:@"mk_cu_addNewDeviceSuccessNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceOnlineState:)
                                                 name:MKCUReceiveDeviceOnlineNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeleteDevice:)
                                                 name:@"mk_cu_deleteDeviceNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceModifyMQTTServer:)
                                                 name:@"mk_cu_deviceModifyMQTTServerSuccessNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(serverManagerStateChanged)
                                                 name:MKCUMQTTSessionManagerStateChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceNetworkState:)
                                                 name:MKCUReceiveDeviceNetStateNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceLwtMessage:)
                                                 name:MKCUReceiveDeviceOfflineNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceResetByButton:)
                                                 name:MKCUReceiveDeviceResetByButtonNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(reloadDeviceTopics)
                                                 name:@"mk_cu_needReloadTopicsNotification"
                                               object:nil];
}
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKIoTLoginService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [[MKCUUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
        MKCUSyncDeviceController *vc = [[MKCUSyncDeviceController alloc] init];
...
- (void)loadSubViews {
    self.defaultTitle = @"MKScannerPro";
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListController", @"cu_menuIcon.png") forState:UIControlStateNormal];
...
- (MKCUAddDeviceView *)addView {
    if (!_addView) {
        _addView = [[MKCUAddDeviceView alloc] init];
    }
    return _addView;
}

- (MKCUEasyShowView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[MKCUEasyShowView alloc] init];
    }
    return _loadingView;
}
