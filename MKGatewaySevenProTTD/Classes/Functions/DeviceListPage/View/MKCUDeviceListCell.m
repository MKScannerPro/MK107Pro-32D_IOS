...
@implementation MKCUDeviceListCell

+ (MKCUDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKCUDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(cu_cellDeleteButtonPressed:)]) {
        [self.delegate cu_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKCUDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKCUDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKCUDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListCell", @"cu_offline_wifi.png");
        return;
    }
    //设备在线
    if (_dataModel.wifiLevel >= -50) {
        //Good
        self.wifiIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListCell", @"cu_good_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel >= -65 && _dataModel.wifiLevel < -50) {
        //Medium
        self.wifiIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListCell", @"cu_medium_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel < -65) {
        //Poor
        self.wifiIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListCell", @"cu_poor_wifi.png");
        return;
    }
}
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListCell", @"cu_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceListCell", @"cu_goNextButton.png");
    }
    return _rightIcon;
}
