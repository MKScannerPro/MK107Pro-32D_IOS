...
@interface MKCUServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCUServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKCUServerForAppModel *dataModel;

@property (nonatomic, strong)MKCUServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKCUServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKCUServerForAppController

- (void)dealloc {
    NSLog(@"MKCUServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKCUMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKCUMQTTDataManager shared] connect];
}
...
#pragma mark - MKCUServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)cu_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cu_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)cu_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)cu_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)cu_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)cu_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)cu_selectedServerParams:(NSString *)fileName {
    [MKCUExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCUServerForAppModel *model = [MKCUServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKCUMQTTDataManager shared].state == MKCUMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKCUMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cu_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKCUExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKCUMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKCUMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUServerForAppController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCUServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCUServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKCUServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCUServerConfigAppFooterViewModel alloc] init];
    }
