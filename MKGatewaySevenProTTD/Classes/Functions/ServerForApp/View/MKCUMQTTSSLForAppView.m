...
@implementation MKCUMQTTSSLForAppViewModel
@end

@interface MKCUMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCUMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate cu_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate cu_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate cu_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cu_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate cu_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCUMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTSSLForAppView", @"cu_switchSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTSSLForAppView", @"cu_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUMQTTSSLForAppView", @"cu_switchUnselectedIcon.png") forState:UIControlStateNormal];
