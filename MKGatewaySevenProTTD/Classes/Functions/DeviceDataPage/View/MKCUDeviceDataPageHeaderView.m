//
//  MKCUDeviceDataPageHeaderView.m
...
@implementation MKCUDeviceDataPageHeaderViewModel
@end

@interface MKCUDeviceDataPageManageBleButton : UIControl
...
@implementation MKCUDeviceDataPageManageBleButton
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceDataPageManageBleButton", @"cu_goNextButton.png");
    }
    return _rightIcon;
}
...
@interface MKCUDeviceDataPageHeaderView ()
...
@property (nonatomic, strong)MKCUDeviceDataPageManageBleButton *manageBleButton;

@end

@implementation MKCUDeviceDataPageHeaderView
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_updateLoadButtonAction)]) {
        [self.delegate cu_updateLoadButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_scannerStatusChanged:)]) {
        [self.delegate cu_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)manageBleButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_manageBleDeviceAction)]) {
        [self.delegate cu_manageBleDeviceAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUDeviceDataPageHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUDeviceDataPageHeaderViewModel.class]) {
        return;
    }
    self.scannerButton.selected = _dataModel.isOn;
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceDataPageHeaderView", @"cu_switchSelectedIcon.png") : LOADICON(@"MKGatewaySevenProTTD", @"MKCUDeviceDataPageHeaderView", @"cu_switchUnselectedIcon.png"));
...
- (MKCUDeviceDataPageManageBleButton *)manageBleButton {
    if (!_manageBleButton) {
        _manageBleButton = [[MKCUDeviceDataPageManageBleButton alloc] init];
