//
//  MKCUFilterByBeaconController.m
...
@interface MKCUFilterByBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByBeaconModel *dataModel;

@end

@implementation MKCUFilterByBeaconController

- (void)dealloc {
    NSLog(@"MKCUFilterByBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByBeaconController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByBeaconModel alloc] initWithPageType:self.pageType];
    }
    return _dataModel;
}
