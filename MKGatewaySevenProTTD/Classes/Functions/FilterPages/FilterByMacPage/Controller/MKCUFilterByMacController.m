...
@interface MKCUFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByMacModel *dataModel;

@end

@implementation MKCUFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByMacController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByMacModel alloc] init];
    }
    return _dataModel;
}
