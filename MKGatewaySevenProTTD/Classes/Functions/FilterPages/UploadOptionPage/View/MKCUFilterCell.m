...
@implementation MKCUFilterCellModel
@end

@interface MKCUFilterCell ()
...
@implementation MKCUFilterCell

+ (MKCUFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUFilterCellIdenty"];
    if (!cell) {
        cell = [[MKCUFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cu_filterValueChanged:index:)]) {
            [self.delegate cu_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCUFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUFilterCellModel.class]) {
        return;
    }
