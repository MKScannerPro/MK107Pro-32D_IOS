...
@interface MKCUUploadOptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUUploadOptionModel *dataModel;

@end

@implementation MKCUUploadOptionController
...
- (void)filterByRawData {
    MKCUFilterByRawDataController *vc = [[MKCUFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUUploadOptionController", @"cu_saveIcon.png")
...
- (MKCUUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUUploadOptionModel alloc] init];
    }
    return _dataModel;
}
