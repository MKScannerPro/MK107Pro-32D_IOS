...
@interface MKCUFilterByNanoBeaconModel ()
...
@implementation MKCUFilterByNanoBeaconModel
...
- (BOOL)readFilterInformation {
    ...
    [MKCUMQTTInterface cu_readFilterByNanoBeaconWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterInformation:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configFilterByNanoBeacon:self.isOn advType:self.triggerType manufactureIDList:list macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
