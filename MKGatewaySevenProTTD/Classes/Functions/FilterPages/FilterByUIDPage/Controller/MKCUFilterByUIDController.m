...
@interface MKCUFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByUIDModel *dataModel;

@end

@implementation MKCUFilterByUIDController

- (void)dealloc {
    NSLog(@"MKCUFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByUIDController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByUIDModel alloc] init];
    }
    return _dataModel;
}
