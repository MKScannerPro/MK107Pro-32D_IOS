//
//  MKCUUploadOptionV2Controller.m
...
@interface MKCUUploadOptionV2Controller ()<UITableViewDelegate,
UITableViewDataSource,
MKNormalSliderCellDelegate,
MKCUFilterCellDelegate>
...
@property (nonatomic, strong)MKCUUploadOptionV2Model *dataModel;

@end

@implementation MKCUUploadOptionV2Controller
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCUFilterCell *cell = [MKCUFilterCell initCellWithTableView:tableView];
...
- (void)cu_filterValueChanged:(NSInteger)dataListIndex index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKCUFilterCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
    if (index == 2) {
        ...
        MKCUFilterCellModel *cellModel = self.section3List[1];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)filterByMACAddress {
    MKCUFilterByMacController *vc = [[MKCUFilterByMacController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByADVName {
    MKCUFilterByAdvNameController *vc = [[MKCUFilterByAdvNameController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByRawData {
    MKCUFilterByRawDataV2Controller *vc = [[MKCUFilterByRawDataV2Controller alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)duplicateDataFilter {
    MKCUDuplicateDataFilterController *vc = [[MKCUDuplicateDataFilterController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataOption {
    MKCUUploadDataOptionController *vc = [[MKCUUploadDataOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataInterval {
    MKCUDataUploadIntervalController *vc = [[MKCUDataUploadIntervalController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSection1Datas {
    MKCUFilterCellModel *cellModel = [[MKCUFilterCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection3Datas {
    MKCUFilterCellModel *cellModel1 = [[MKCUFilterCellModel alloc] init];
    cellModel1.index = 1;
    ...
    MKCUFilterCellModel *cellModel2 = [[MKCUFilterCellModel alloc] init];
    cellModel2.index = 2;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUUploadOptionV2Controller", @"cu_saveIcon.png")
...
- (MKCUUploadOptionV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUUploadOptionV2Model alloc] init];
    }
    return _dataModel;
}
