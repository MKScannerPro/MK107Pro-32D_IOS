...
@interface MKCUFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByTagModel *dataModel;

@end

@implementation MKCUFilterByTagController

- (void)dealloc {
    NSLog(@"MKCUFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByTagController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByTagModel alloc] init];
    }
    return _dataModel;
}
