//
//  MKCUFilterByAdvNameController.m
...
@interface MKCUFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByAdvNameModel *dataModel;

@end

@implementation MKCUFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKCUFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByAdvNameController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
