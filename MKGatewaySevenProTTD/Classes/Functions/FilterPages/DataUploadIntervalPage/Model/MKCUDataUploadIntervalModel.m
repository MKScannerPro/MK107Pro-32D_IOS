...
@interface MKCUDataUploadIntervalModel ()
...
@implementation MKCUDataUploadIntervalModel
...
- (BOOL)readUploadInterval {
    ...
    [MKCUMQTTInterface cu_readUploadDataIntervalWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configUploadInterval {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configUploadDataInterval:[self.interval integerValue] macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
