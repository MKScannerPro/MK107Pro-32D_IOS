...
@interface MKCUFilterByURLController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByURLModel *dataModel;

@end

@implementation MKCUFilterByURLController

- (void)dealloc {
    NSLog(@"MKCUFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByURLController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByURLModel alloc] init];
    }
    return _dataModel;
}
