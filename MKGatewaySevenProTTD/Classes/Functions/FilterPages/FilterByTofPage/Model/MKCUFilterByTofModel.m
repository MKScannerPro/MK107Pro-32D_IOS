...
@interface MKCUFilterByTofModel ()
...
@implementation MKCUFilterByTofModel
...
- (BOOL)readFilterByTof {
    ...
    [MKCUMQTTInterface cu_readFilterByTofWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTof:(NSArray <NSString *>*)codeList {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configFilterByTofList:codeList isOn:self.isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
