...
@interface MKCUFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByTofModel *dataModel;

@end

@implementation MKCUFilterByTofController

- (void)dealloc {
    NSLog(@"MKCUFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByTofController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByTofModel alloc] init];
    }
    return _dataModel;
}
