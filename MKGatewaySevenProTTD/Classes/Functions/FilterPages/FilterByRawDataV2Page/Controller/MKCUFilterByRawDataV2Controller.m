//
//  MKCUFilterByRawDataV2Controller.m
...
@interface MKCUFilterByRawDataV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByRawDataV2Model *dataModel;

@end

@implementation MKCUFilterByRawDataV2Controller

- (void)dealloc {
    NSLog(@"MKCUFilterByRawDataV2Controller销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKCUFilterByBeaconController *vc = [[MKCUFilterByBeaconController alloc] init];
        vc.pageType = mk_cu_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKCUFilterByUIDController *vc = [[MKCUFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKCUFilterByURLController *vc = [[MKCUFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKCUFilterByTLMController *vc = [[MKCUFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKCUFilterByButtonController *vc = [[MKCUFilterByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP-Tag
        MKCUFilterByTagController *vc = [[MKCUFilterByTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //PIR
        MKCUFilterByPirController *vc = [[MKCUFilterByPirController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 3) {
        //MK TOF
        MKCUFilterByTofController *vc = [[MKCUFilterByTofController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 4) {
        //Nano Beacon
        MKCUFilterByNanoBeaconController *vc = [[MKCUFilterByNanoBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 5) {
        //Other
        MKCUFilterByOtherController *vc = [[MKCUFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configFilterBXPDeviceInfo:(BOOL)isOn {
    ...
    [MKCUMQTTInterface cu_configFilterFilterBXPDeviceInfo:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPACC:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_configFilterBXPAcc:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCUMQTTInterface cu_configFilterBXPTH:isOn macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (MKCUFilterByRawDataV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByRawDataV2Model alloc] init];
    }
    return _dataModel;
}
