...
@interface MKCUFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByPirModel *dataModel;

@end

@implementation MKCUFilterByPirController

- (void)dealloc {
    NSLog(@"MKCUFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByPirController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByPirModel alloc] init];
    }
    return _dataModel;
}
