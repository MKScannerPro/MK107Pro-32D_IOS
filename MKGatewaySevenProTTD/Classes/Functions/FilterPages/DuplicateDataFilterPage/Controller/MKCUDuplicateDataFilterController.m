//
//  MKCUDuplicateDataFilterController.m
...
@interface MKCUDuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUDuplicateDataFilterModel *dataModel;

@end

@implementation MKCUDuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKCUDuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUDuplicateDataFilterController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUDuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUDuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
