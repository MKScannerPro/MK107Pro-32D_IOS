...
@implementation MKCUFilterRawAdvDataModel
...
@interface MKCUFilterByOtherModel ()
...
@implementation MKCUFilterByOtherModel
...
- (void)configWithRawDataList:(NSArray <MKCUFilterRawAdvDataModel *>*)list
...
- (BOOL)readFilterByOther {
    ...
    [MKCUMQTTInterface cu_readFilterOtherDatasWithMacAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByOther:(NSArray <MKCUFilterRawAdvDataModel *>*)list relationship:(mk_cu_filterByOther)relationship {
    __block BOOL success = NO;
    [MKCUMQTTInterface cu_configFilterByOtherDatas:self.isOn relationship:relationship rawDataList:list macAddress:[MKCUDeviceModeManager shared].macAddress topic:[MKCUDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
