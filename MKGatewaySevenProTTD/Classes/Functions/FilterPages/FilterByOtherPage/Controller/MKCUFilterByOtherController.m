...
@interface MKCUFilterByOtherController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCUFilterByOtherModel *dataModel;

@end

@implementation MKCUFilterByOtherController

- (void)dealloc {
    NSLog(@"MKCUFilterByOtherController销毁");
}
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        ...
        MKCUFilterRawAdvDataModel *model = [[MKCUFilterRawAdvDataModel alloc] init];
...
- (mk_cu_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_cu_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_cu_filterByOther_AB;
        }
        return mk_cu_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_cu_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_cu_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_cu_filterByOther_AOrBOrC;
        }
    }
    return mk_cu_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUFilterByOtherController", @"cu_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCUFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCUFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
