...
@implementation MKCUCreatScannerProDeviceModel
...
@interface MKCUNetworkService ()

@property (nonatomic, strong)NSURLSessionDataTask *addScannerProDeviceTask;

@end

@implementation MKCUNetworkService

+ (instancetype)share {
    static dispatch_once_t t;
    static MKCUNetworkService *service = nil;
    dispatch_once(&t, ^{
        service = [[MKCUNetworkService alloc] init];
    });
    return service;
}

- (void)addScannerProDevicesToCloud:(NSArray <MKCUCreatScannerProDeviceModel *>*)deviceList
                             isHome:(BOOL)isHome
    ...
    for (MKCUCreatScannerProDeviceModel *deviceModel in deviceList) {
