...
@interface MKCUBaseViewController ()

@end

@implementation MKCUBaseViewController

- (void)dealloc {
    NSLog(@"MKCUBaseViewController销毁");
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceOffline:)
                                                 name:MKCUDeviceModelOfflineNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)processOfflineWithMacAddress:(NSString *)macAddress {
    if (![macAddress isEqualToString:[MKCUDeviceModeManager shared].macAddress]) {
        return;
    }
