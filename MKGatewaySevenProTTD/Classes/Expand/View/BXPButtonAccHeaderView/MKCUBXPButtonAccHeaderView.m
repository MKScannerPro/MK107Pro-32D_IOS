...
@interface MKCUBXPButtonAccHeaderView ()
...
@implementation MKCUBXPButtonAccHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_bxpButtonAccHeaderView_syncButtonPressed:)]) {
        [self.delegate cu_bxpButtonAccHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_bxpButtonAccHeaderView_exportButtonPressed)]) {
        [self.delegate cu_bxpButtonAccHeaderView_exportButtonPressed];
    }
}
...
- (void)updateSyncStatus:(BOOL)isOn {
    [self.syncIcon.layer removeAnimationForKey:@"mk_cu_syncAnimation"];
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPButtonAccHeaderView", @"cu_threeAxisAcceLoadingIcon.png");
    }
    return _syncIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewaySevenProTTD", @"MKCUBXPButtonAccHeaderView", @"cu_slotExportEnableIcon.png") forState:UIControlStateNormal];
