...
@implementation MKCUUserCredentialsViewModel
@end

@interface MKCUUserCredentialsView ()
...
@implementation MKCUUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCUUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cu_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate cu_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cu_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate cu_mqtt_userCredentials_passwordChanged:text];
            }
        };
