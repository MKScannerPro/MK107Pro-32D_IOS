...
@implementation MKCUPressEventCountCellModel
@end

@interface MKCUPressEventCountCell ()
...
@implementation MKCUPressEventCountCell

+ (MKCUPressEventCountCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUPressEventCountCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUPressEventCountCellIdenty"];
    if (!cell) {
        cell = [[MKCUPressEventCountCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUPressEventCountCellIdenty"];
    }
    return cell;
}
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cu_pressEventCountCell_clearButtonPressed:)]) {
        [self.delegate cu_pressEventCountCell_clearButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCUPressEventCountCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUPressEventCountCellModel.class]) {
        return;
    }
