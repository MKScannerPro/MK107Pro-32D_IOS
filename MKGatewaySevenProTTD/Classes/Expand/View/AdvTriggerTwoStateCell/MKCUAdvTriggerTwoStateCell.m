...
@implementation MKCUAdvTriggerTwoStateCellModel
@end

@interface MKCUAdvTriggerTwoStateCell ()
...
@implementation MKCUAdvTriggerTwoStateCell

+ (MKCUAdvTriggerTwoStateCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUAdvTriggerTwoStateCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUAdvTriggerTwoStateCellIdenty"];
    if (!cell) {
        cell = [[MKCUAdvTriggerTwoStateCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUAdvTriggerTwoStateCellIdenty"];
    }
    return cell;
}
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_advNormalCell_setPressed:beforeInterval:beforeTxPower:afterInterval:afterTxPower:)]) {
        [self.delegate cu_advNormalCell_setPressed:self.dataModel.index
...
- (void)txPowerButtonPressed:(UIButton *)btn {
    ...
    [pickView showPickViewWithDataList:self.txPowerList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cu_advNormalCell_txPowerChanged:before:txPower:)]) {
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCUAdvTriggerTwoStateCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUAdvTriggerTwoStateCellModel.class]) {
        return;
    }
