...
@implementation MKCUBXPAdvParamsCellModel

- (CGFloat)fetchCellHeight {
    if (self.slotType == MKCUBXPAdvParamsCellSlotTypeNoData) {
        return 44.f;
    }
    return 150.f;
}
...
@interface MKCUBXPAdvParamsCell ()
...
@implementation MKCUBXPAdvParamsCell

+ (MKCUBXPAdvParamsCell *)initCellWithTableView:(UITableView *)tableView {
    MKCUBXPAdvParamsCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCUBXPAdvParamsCellIdenty"];
    if (!cell) {
        cell = [[MKCUBXPAdvParamsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCUBXPAdvParamsCellIdenty"];
    }
    return cell;
}
...
- (void)layoutSubviews {
    ...
    if (self.dataModel.slotType == MKCUBXPAdvParamsCellSlotTypeNoData) {
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cu_BXPAdvParamsCell_setPressedWithSlotIndex:interval:txPower:)]) {
        [self.delegate cu_BXPAdvParamsCell_setPressedWithSlotIndex:self.dataModel.slotIndex
...
#pragma mark - setter
- (void)setDataModel:(MKCUBXPAdvParamsCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCUBXPAdvParamsCellModel.class]) {
        return;
    }
    ...
    if (_dataModel.slotType == MKCUBXPAdvParamsCellSlotTypeNoData) {
        [self setNeedsLayout];
        return;
    }
...
- (NSString *)getSlotTypeString:(MKCUBXPAdvParamsCellSlotType)slotType {
    if (slotType == MKCUBXPAdvParamsCellSlotTypeUID) {
        return @"UID";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeURL) {
        return @"URL";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeTLM) {
        return @"TLM";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeEID) {
        return @"EID";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeDeviceInfo) {
        return @"Device info";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeiBeacon) {
        return @"iBeacon";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeAccelerometer) {
        return @"ACC";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeHT) {
        return @"TH";
    }
    if (slotType == MKCUBXPAdvParamsCellSlotTypeTag) {
        return @"Tag";
    }
    return @"No data";
}

- (NSString *)getTriggerTypeString:(MKCUBXPAdvParamsCellTriggerType)triggerType {
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeNull) {
        return @"Off";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeTemperature) {
        return @"Temperature";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeHumidity) {
        return @"Humidity";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeDoubleClick) {
        return @"Double click button";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeTripleClick) {
        return @"Triple click button";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeMove) {
        return @"Devices moves";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeLight) {
        return @"Light";
    }
    if (triggerType == MKCUBXPAdvParamsCellTriggerTypeSingleClick) {
